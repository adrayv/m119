...
def button_publish(button, mac):
    if button==0:
        urllib2.urlopen("https://sheltered-brook-80388.herokuapp.com/update?player=1&action=right").read()
        motion="right"
        messageSent="event "+motion + " @ " + hexi_addr
        myAWSIoTMQTTClient.publish(topic,messageSent,1)
    if button==1:
        urllib2.urlopen("https://sheltered-brook-80388.herokuapp.com/update?player=1&action=left").read()
        motion="left"
        messageSent="event "+motion + " @ " + hexi_addr
        myAWSIoTMQTTClient.publish(topic,messageSent,1)
    if button==2:
        urllib2.urlopen("https://sheltered-brook-80388.herokuapp.com/update?player=1&action=up").read()
        motion="up"
        messageSent="event "+motion + " @ " + hexi_addr
        myAWSIoTMQTTClient.publish(topic,messageSent,1)    
    if button==3:
        urllib2.urlopen("https://sheltered-brook-80388.herokuapp.com/update?player=1&action=down").read()
        motion="down"
        messageSent="event "+motion + " @ " + hexi_addr
        myAWSIoTMQTTClient.publish(topic,messageSent,1)
    if button==4:
        urllib2.urlopen("https://sheltered-brook-80388.herokuapp.com/update?player=1&action=buzz").read()
        motion="HandRaised"
        messageSent="event "+motion + " @ " + hexi_addr
        myAWSIoTMQTTClient.publish(topic,messageSent,1)
